...
class Home extends Component {
  ...
  componentDidMount = () => {
    axios
      .get("http://localhost:8081/board/view", { headers })
      .then(returnData => {
        this.setState({
  ...
  addBoard = () => {
    ...
    axios
      .post("http://localhost:8081/board/insert", send_param)
      .then(returnData => {
  ...
  deleteBoard = (board_no, member_no) => {
    ...
    if (login_no === member_no) {
      ...
      axios
        .post("http://localhost:8081/board/delete", send_param)
        .then(returnData => {
          alert(returnData.data.message);
          window.location.reload();
        });
                ...
                <Grid item key={posts.b_no} xs={12} md={6}>
                  <Card className={classes.card}>
                    ...
                    <CardContent className={classes.cardContent}>
                      ...
                      <Typography>작성자:{posts.name}</Typography>
                    </CardContent>
                    <CardActions>
                      ...
                      <Button
                        ...
                        color="primary"
                        style={
                          login_no === posts.m_no ? writeStyle : deleteStyle
                        }
                        onClick={() => this.deleteBoard(posts.b_no, posts.m_no)}
                      >
                        Delete
                      </Button>
                    </CardActions>
